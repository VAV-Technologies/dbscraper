...
if (global.scrapeQueue) {
  global.scrapeQueue.process('scrape', async (job) => {
    const { jobId, url, expectedCount, options, proxies = [] } = job.data;

    try {
      const scrapingJob = await ScrapingJob.findOne({ jobId });
      if (!scrapingJob) {
        throw new Error('Job not found in database');
      }

      // Check if job was paused
      if (scrapingJob.status === 'paused') {
        global.logger.info(`Job ${jobId} is paused, not starting`);
        return;
      }

      scrapingJob.status = 'in_progress';
      await scrapingJob.save();

      // Use proxies from request body, fallback to environment variable
      const proxyList = proxies.length > 0 ? proxies : (process.env.PROXIES ? process.env.PROXIES.split(',') : []);

      global.logger.info(`Starting scrape job ${jobId} with ${proxyList.length} proxies`);

      const scraper = new DNBScraperStealth({
        ...options,
        proxies: proxyList,
        headless: process.env.HEADLESS !== 'false',
        minDelay: options.minDelay || 5000,
        maxDelay: options.maxDelay || 15000
      });
      activeScrapers.set(jobId, scraper);

      await scraper.initialize();

      const { resultCount, errors } = await scraper.scrapeDirectory(url, {
        expectedCount,
        maxPages: options.maxPages,
        jobId
      });

      // Get final count from CompanyResult collection
      const finalCount = await CompanyResult.countDocuments({ jobId });

      scrapingJob.errors = errors;
      scrapingJob.status = errors.length > 0 ? 'completed_with_errors' : 'completed';
      scrapingJob.completedAt = new Date();
      scrapingJob.progress = {
        companiesScraped: finalCount,
      ...
      global.logger.error(`Scraping job ${jobId} failed:`, error);
      
      const scrapingJob = await ScrapingJob.findOne({ jobId });
      if (scrapingJob) {
        scrapingJob.status = 'failed';
        scrapingJob.errors.push({
          error: error.message,
          timestamp: new Date()
        });
        scrapingJob.completedAt = new Date();
